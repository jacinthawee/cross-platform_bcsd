...

{
  ...
  if (type == 0x72) {
    if (m_length != 0x24) {
      ERR_put_error(4,0x75,0x83,"rsa_sign.c",0x60);
      return 0;
    }
    iVar1 = RSA_size(rsa);
    if (iVar1 < 0x2f) goto LAB_000971ca;
    ptr = (ASN1_OBJECT *)0x0;
  }
  else {
    ...
    if ((local_48.algor)->algorithm == (ASN1_OBJECT *)0x0) {
      ERR_put_error(4,0x75,0x75,"rsa_sign.c",0x6a);
      return 0;
    }
    if ((local_48.algor)->algorithm->length == 0) {
      ERR_put_error(4,0x75,0x74,"rsa_sign.c",0x6f);
      return 0;
    }
...
LAB_000971ca:
      ERR_put_error(4,0x75,0x70,"rsa_sign.c",0x7f);
      return 0;
    }
    ptr = (ASN1_OBJECT *)CRYPTO_malloc(iVar1 + 1,"rsa_sign.c",0x83);
    if (ptr == (ASN1_OBJECT *)0x0) {
      ERR_put_error(4,0x75,0x41,"rsa_sign.c",0x86);
      return 0;
    }
    local_4c = ptr;
