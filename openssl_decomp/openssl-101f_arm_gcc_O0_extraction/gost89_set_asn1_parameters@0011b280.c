...

{
  ...
  if (ppAVar1 == (ASN1_STRING **)0x0) {
    ERR_GOST_error(0x6d,0x76,"gost_crypt.c",0x1ad);
    return (void *)0x0;
  }
  ...
  if (iVar2 != 0) {
    ...
    data = CRYPTO_malloc(iVar2,"gost_crypt.c",0x1ba);
    local_1c = data;
    if (data == (void *)0x0) {
      GOST_CIPHER_PARAMS_free(ppAVar1);
      ERR_GOST_error(0x6d,0x76,"gost_crypt.c",0x1be);
    }
    else {
      ...
      CRYPTO_free(data);
      ERR_GOST_error(0x6d,0x76,"gost_crypt.c",0x1c9);
      data = (void *)0x0;
    }
    return data;
  }
