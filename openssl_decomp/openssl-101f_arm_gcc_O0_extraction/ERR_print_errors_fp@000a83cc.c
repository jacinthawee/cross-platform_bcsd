...
void ERR_print_errors_fp(void *param_1)

{
  ...
  iStack_24 = __TMC_END__;
  CRYPTO_THREADID_current(&CStack_116c);
  uVar1 = CRYPTO_THREADID_hash(&CStack_116c);
  do {
    e = ERR_get_error_line_data(&pcStack_117c,&iStack_1174,&pcStack_1178,&iStack_1170);
    if (e == 0) break;
    ERR_error_string_n(e,acStack_1124,0x100);
    pcVar3 = pcStack_1178;
    if (-1 < iStack_1170 << 0x1e) {
      pcVar3 = "";
    }
    BIO_snprintf(acStack_1024,0x1000,"%lu:%s:%s:%d:%s\n",uVar1,acStack_1124,pcStack_117c,iStack_1174
                 ,pcVar3);
    type = BIO_s_file();
