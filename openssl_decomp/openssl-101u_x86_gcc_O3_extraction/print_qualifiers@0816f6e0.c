...

{
  ...
  while( true ) {
    ...
    if (iVar5 == 0xa4) {
      BIO_printf(param_1,"%*sCPS: %s\n",param_3,&DAT_081eca46,ppAVar6[1]->nid);
    }
    else if (iVar5 == 0xa5) {
      ...
      if (piVar2 != (int *)0x0) {
        BIO_printf(param_1,"%*sOrganization: %s\n",iVar4,&DAT_081eca46,*(undefined4 *)(*piVar2 + 8))
        ;
        ...
        iVar5 = 0;
        BIO_printf(param_1,"%*sNumber%s: ",iVar4,&DAT_081eca46,pcVar7);
        while( true ) {
      ...
      ppcVar3 = pAVar1->ln;
      if (ppcVar3 != (char **)0x0) {
        BIO_printf(param_1,"%*sExplicit Text: %s\n",iVar4,&DAT_081eca46,ppcVar3[2]);
      }
    }
    else {
      BIO_printf(param_1,"%*sUnknown Qualifier: ",iVar4,&DAT_081eca46);
