...
LAB_080e5d87:
  ...
  uVar5 = (uint)userKey[4] << 0x18 ^ (uint)userKey[5] << 0x10 ^ (uint)userKey[7] ^
          (uint)userKey[6] << 8;
  key->rd_key[1] = uVar5;
  local_18 = (uint)userKey[9] << 0x10 ^ (uint)userKey[8] << 0x18 ^ (uint)userKey[0xb] ^
             (uint)userKey[10] << 8;
  key->rd_key[2] = local_18;
  bVar1 = userKey[0xc];
  ...
  if (bits == 0x80) {
    ...
    key->rd_key[0x2a] = local_18;
    key->rd_key[0x2b] = uVar3 ^ local_18;
  }
  else {
    key->rd_key[4] =
         (uint)userKey[0x12] << 8 ^
    ...
    if (bits == 0xc0) {
      ...
      key->rd_key[0x32] = local_18;
      key->rd_key[0x33] = local_18 ^ uVar3;
    }
    else {
      key->rd_key[6] =
           (uint)userKey[0x1a] << 8 ^
      ...
      if (bits == 0x100) {
        ...
        while( true ) {
          uVar2 = key->rd_key[7];
          uVar4 = *(uint *)(Te3 + (uVar2 >> 8 & 0xff) * 4) & 0xff0000 ^
                  uVar4 ^ *(uint *)(rcon + local_1c) ^ *(uint *)(Te0 + (uVar2 & 0xff) * 4) & 0xff00
                  ^ (uint)(byte)Te1[(uVar2 >> 0x18) * 4] ^
                  *(uint *)(Te2 + (uVar2 >> 0x10 & 0xff) * 4) & 0xff000000;
          uVar5 = uVar5 ^ uVar4;
          local_18 = local_18 ^ uVar5;
