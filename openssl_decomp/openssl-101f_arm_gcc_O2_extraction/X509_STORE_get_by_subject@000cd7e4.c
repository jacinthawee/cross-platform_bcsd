...

{
  ...
  pXVar9 = vs->ctx;
  CRYPTO_lock(9,0xb,"x509_lu.c",0x129);
  p_Var6 = (_STACK *)pXVar9->objs;
  iVar1 = x509_object_idx_cnt(p_Var6,type,name,0);
  if (iVar1 == -1) {
    piVar2 = (int *)0x0;
    CRYPTO_lock(10,0xb,"x509_lu.c",299);
  }
  else {
    piVar2 = (int *)sk_value(p_Var6,iVar1);
    CRYPTO_lock(10,0xb,"x509_lu.c",299);
    if (type != 2 && piVar2 != (int *)0x0) goto LAB_000cd886;
  }
...
LAB_000cd886:
  ...
  if (iVar1 == 1) {
    CRYPTO_add_lock(&pXVar5->references,1,3,"x509_lu.c",0x195);
    return 1;
  }
  if (iVar1 == 2) {
    CRYPTO_add_lock(&pXVar5->valid,1,6,"x509_lu.c",0x198);
    return 1;
  }
  return 1;
}
