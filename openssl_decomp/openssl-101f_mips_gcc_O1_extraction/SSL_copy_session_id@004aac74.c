...

{
  ...
  if (from->cert == (cert_st *)0x0) {
    ...
    CRYPTO_add_lock((int *)(from->cert + 0x90),1,0xd,"ssl_lib.c",0x370);
    to->cert = from->cert;
  }
  if (pcVar1 != (cert_st *)0x0) {
    ssl_cert_free(pcVar1);
  }
  UNRECOVERED_JUMPTABLE = PTR_memcpy_006aabf4;
  if (from->sid_ctx_length < 0x21) {
    to->sid_ctx_length = from->sid_ctx_length;
                    /* WARNING: Could not recover jumptable at 0x004aad6c. Too many branches */
