...
/* WARNING: Heritage AFTER dead removal. Example location: r0x00194c40 : 0x0007bdc2 */
/* WARNING: Heritage AFTER dead removal. Example location: s0xffffffb4 : 0x0007bad0 */
...

{
  ...
  if ((out == (uchar *)0x0 || in == (uchar *)0x0) || (ivec == (uchar *)0x0 || key == (AES_KEY *)0x0)
    ...
    OpenSSLDie("aes_ige.c",0xde,"in && out && key && ivec");
  }
  if (1 < (uint)enc) {
    OpenSSLDie("aes_ige.c",0xdf,"(AES_ENCRYPT == enc) || (AES_DECRYPT == enc)");
  }
  if ((length & 0xf) != 0) {
    OpenSSLDie("aes_ige.c",0xe0,"(length % AES_BLOCK_SIZE) == 0");
  }
  if (enc == 1) {
