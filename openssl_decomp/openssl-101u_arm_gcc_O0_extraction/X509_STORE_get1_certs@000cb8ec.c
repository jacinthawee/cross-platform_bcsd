...

{
  ...
  st_00 = sk_new_null();
  CRYPTO_lock(9,0xb,"x509_lu.c",0x1e0);
  iVar1 = x509_object_idx_cnt(st->ctx->objs,1,nm,&local_2c);
  if (iVar1 < 0) {
    CRYPTO_lock(10,0xb,"x509_lu.c",0x1e8);
    ...
    if (iVar1 != 0) {
      ...
      CRYPTO_lock(9,0xb,"x509_lu.c",0x1ee);
      iVar1 = x509_object_idx_cnt(st->ctx->objs,1,nm,&local_2c);
      if (-1 < iVar1) goto LAB_000cb924;
      CRYPTO_lock(10,0xb,"x509_lu.c",0x1f1);
    }
    sk_free(st_00);
    st_00 = (_STACK *)0x0;
  }
...
LAB_000cb924:
    if (0 < local_2c) {
      ...
      do {
        ...
        if (iVar3 == 0) {
          CRYPTO_lock(10,0xb,"x509_lu.c",0x1fb);
          X509_free(a);
    ...
    CRYPTO_lock(10,0xb,"x509_lu.c",0x201);
  }
