...

{
  ...
  if (param_2 == 1) {
    ...
    iVar1 = CRYPTO_add_lock(&param_4->references,1,0x15,"e_sureware.c",399);
    if (iVar1 < 2) {
      if (SUREWARE_lib_error_code == (BIO *)0x0) {
        SUREWARE_lib_error_code = (BIO *)ERR_get_next_error_library();
      }
      param_4 = SUREWARE_lib_error_code;
      ERR_put_error((int)SUREWARE_lib_error_code,100,100,"e_sureware.c",0x192);
    }
    else {
      logstream = param_4;
    }
  ...
  ERR_put_error((int)SUREWARE_lib_error_code,100,0x77,"e_sureware.c",0x1a4);
  return 0;
}
