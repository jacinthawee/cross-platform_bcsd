
int DSA_generate_parameters_ex
              (DSA *dsa,int bits,uchar *seed,int seed_len,int *counter_ret,ulong *h_ret,BN_GENCB *cb
              )

{
  _func_1740 *UNRECOVERED_JUMPTABLE;
  int iVar1;
  EVP_MD *pEVar2;
  undefined4 local_28;
  
  UNRECOVERED_JUMPTABLE = dsa->meth->dsa_paramgen;
  if (UNRECOVERED_JUMPTABLE != (_func_1740 *)0x0) {
                    /* WARNING: Could not recover jumptable at 0x0810e8f7. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    iVar1 = (*UNRECOVERED_JUMPTABLE)(dsa,bits,seed,seed_len,counter_ret,h_ret,cb);
    return iVar1;
  }
  if (bits < 0x800) {
    pEVar2 = EVP_sha1();
    local_28 = 0xa0;
  }
  else {
    pEVar2 = EVP_sha256();
    local_28 = 0x100;
  }
  iVar1 = dsa_builtin_paramgen(dsa,bits,local_28,pEVar2,seed,seed_len,0,counter_ret,h_ret,cb);
  return iVar1;
}

