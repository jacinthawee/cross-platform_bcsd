
int * v2i_idp(undefined4 param_1,X509V3_CTX *param_2,undefined4 param_3)

{
  undefined *puVar1;
  int *piVar2;
  int iVar3;
  CONF_VALUE *value;
  X509_NAME *nm;
  stack_st_CONF_VALUE *psVar4;
  undefined4 *puVar5;
  int iVar6;
  char *pcVar7;
  stack_st_X509_NAME_ENTRY *psVar8;
  GENERAL_NAMES *pGVar9;
  char *line;
  
  piVar2 = (int *)(*(code *)PTR_ASN1_item_new_006a9778)(ISSUING_DIST_POINT_it);
  if (piVar2 == (int *)0x0) {
    (*(code *)PTR_ERR_put_error_006a9030)(0x22,0x9d,0x41,"v3_crld.c",0x1f5);
LAB_0059582c:
    (*(code *)PTR_ASN1_item_free_006a977c)(piVar2,ISSUING_DIST_POINT_it);
    return (int *)0x0;
  }
  iVar6 = 0;
  iVar3 = (*(code *)PTR_sk_num_006a7f2c)(param_3);
  if (0 < iVar3) {
    do {
      value = (CONF_VALUE *)(*(code *)PTR_sk_value_006a7f24)(param_3,iVar6);
      pcVar7 = value->name;
      line = value->value;
      iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"fullname");
      if (iVar3 == 0) {
        if (*line == '@') {
          psVar4 = X509V3_get_section(param_2,line + 1);
        }
        else {
          psVar4 = X509V3_parse_list(line);
        }
        if (psVar4 == (stack_st_CONF_VALUE *)0x0) {
          (*(code *)PTR_ERR_put_error_006a9030)(0x22,0x9c,0x96,"v3_crld.c",0x68);
        }
        else {
          pGVar9 = v2i_GENERAL_NAMES((X509V3_EXT_METHOD *)0x0,param_2,psVar4);
          if (*line == '@') {
            X509V3_section_free(param_2,psVar4);
          }
          else {
            (*(code *)PTR_sk_pop_free_006a8158)(psVar4,PTR_X509V3_conf_free_006a8c38);
          }
          if (pGVar9 != (GENERAL_NAMES *)0x0) {
            iVar3 = *piVar2;
            psVar8 = (stack_st_X509_NAME_ENTRY *)0x0;
joined_r0x005957c8:
            if (iVar3 == 0) {
              puVar5 = (undefined4 *)(*(code *)PTR_ASN1_item_new_006a9778)(DIST_POINT_NAME_it);
              *piVar2 = (int)puVar5;
              if (puVar5 != (undefined4 *)0x0) {
                if (pGVar9 == (GENERAL_NAMES *)0x0) {
                  puVar5[1] = psVar8;
                  *puVar5 = 1;
                }
                else {
                  puVar5[1] = pGVar9;
                  *puVar5 = 0;
                }
                goto LAB_00595710;
              }
            }
            else {
              (*(code *)PTR_ERR_put_error_006a9030)(0x22,0x9e,0xa0,"v3_crld.c",0xa5);
            }
            if (pGVar9 != (GENERAL_NAMES *)0x0) {
              (*(code *)PTR_sk_pop_free_006a8158)(pGVar9,PTR_GENERAL_NAME_free_006a84a4);
            }
LAB_00595810:
            if (psVar8 != (stack_st_X509_NAME_ENTRY *)0x0) {
              (*(code *)PTR_sk_pop_free_006a8158)(psVar8,PTR_X509_NAME_ENTRY_free_006a8250);
            }
          }
        }
        goto LAB_0059582c;
      }
      iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"relativename");
      if (iVar3 == 0) {
        nm = (X509_NAME *)(*(code *)PTR_X509_NAME_new_006a8238)();
        if (nm != (X509_NAME *)0x0) {
          psVar4 = X509V3_get_section(param_2,value->value);
          if (psVar4 != (stack_st_CONF_VALUE *)0x0) {
            iVar3 = X509V3_NAME_from_section(nm,psVar4,0x1001);
            X509V3_section_free(param_2,psVar4);
            puVar1 = PTR_X509_NAME_free_006a80f8;
            psVar8 = nm->entries;
            nm->entries = (stack_st_X509_NAME_ENTRY *)0x0;
            (*(code *)puVar1)(nm);
            if ((iVar3 != 0) && (iVar3 = (*(code *)PTR_sk_num_006a7f2c)(psVar8), 0 < iVar3)) {
              iVar3 = (*(code *)PTR_sk_num_006a7f2c)(psVar8);
              iVar3 = (*(code *)PTR_sk_value_006a7f24)(psVar8,iVar3 + -1);
              if (*(int *)(iVar3 + 8) == 0) {
                iVar3 = *piVar2;
                pGVar9 = (GENERAL_NAMES *)0x0;
                goto joined_r0x005957c8;
              }
              (*(code *)PTR_ERR_put_error_006a9030)(0x22,0x9e,0xa1,"v3_crld.c",0x9b);
            }
            goto LAB_00595810;
          }
          (*(code *)PTR_ERR_put_error_006a9030)(0x22,0x9e,0x96,"v3_crld.c",0x8a);
        }
        goto LAB_0059582c;
      }
      iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"onlyuser");
      if (iVar3 == 0) {
        iVar3 = X509V3_get_value_bool(value,piVar2 + 1);
      }
      else {
        iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"onlyCA");
        if (iVar3 == 0) {
          iVar3 = X509V3_get_value_bool(value,piVar2 + 2);
        }
        else {
          iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"onlyAA");
          if (iVar3 == 0) {
            iVar3 = X509V3_get_value_bool(value,piVar2 + 5);
          }
          else {
            iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"indirectCRL");
            if (iVar3 != 0) {
              iVar3 = (*(code *)PTR_strcmp_006aac20)(pcVar7,"onlysomereasons");
              if (iVar3 == 0) {
                iVar3 = set_reasons(piVar2 + 3,line);
                if (iVar3 != 0) goto LAB_00595710;
              }
              else {
                (*(code *)PTR_ERR_put_error_006a9030)(0x22,0x9d,0x6a,"v3_crld.c",0x1ed);
                (*(code *)PTR_ERR_add_error_data_006a9264)
                          (6,"section:",value->section,",name:",value->name,",value:",value->value);
              }
              goto LAB_0059582c;
            }
            iVar3 = X509V3_get_value_bool(value,piVar2 + 4);
          }
        }
      }
      if (iVar3 == 0) goto LAB_0059582c;
LAB_00595710:
      iVar6 = iVar6 + 1;
      iVar3 = (*(code *)PTR_sk_num_006a7f2c)(param_3);
    } while (iVar6 < iVar3);
  }
  return piVar2;
}

