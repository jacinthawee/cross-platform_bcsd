...

{
  ...
  if (5 < nid - 0x15U) {
    ERR_put_error(0x21,0x6e,0x70,"pk7_lib.c",0xdd);
    goto LAB_0817c9eb;
  }
  a->type = pAVar1;
  switch(nid) {
  case 0x15:
    ...
    (a->d).data = pAVar3;
    goto joined_r0x0817c9bd;
  case 0x16:
    pPVar8 = PKCS7_SIGNED_new();
    (a->d).sign = pPVar8;
    if (pPVar8 == (PKCS7_SIGNED *)0x0) goto LAB_0817c9eb;
...
LAB_0817c948:
  ...
  if (iVar5 == 0x16) {
    ...
    if (iVar5 != 0x19) {
      ERR_put_error(0x21,0x6d,0x70,"pk7_lib.c",0x97);
LAB_0817c9eb:
      PKCS7_free(a);
      return 0;
    }
