
/* WARNING: Removing unreachable block (ram,0x0810a0d9) */

size_t RSA_eay_private_encrypt(BN_CTX *param_1,uchar *param_2,void *param_3,int param_4,int param_5)

{
  ulong **ppuVar1;
  BN_CTX *ctx;
  BIGNUM *ret;
  BIGNUM *n;
  int iVar2;
  uchar *to;
  BIGNUM *pBVar3;
  uint uVar4;
  BN_MONT_CTX *pBVar5;
  size_t len;
  size_t sVar6;
  int in_GS_OFFSET;
  int iVar7;
  char *ctx_00;
  BIGNUM *local_50;
  BN_BLINDING *local_4c;
  BIGNUM *local_48;
  BIGNUM local_34;
  int local_20;
  
  local_20 = *(int *)(in_GS_OFFSET + 0x14);
  ctx = BN_CTX_new();
  if (ctx == (BN_CTX *)0x0) {
    sVar6 = 0xffffffff;
    goto LAB_08109e68;
  }
  BN_CTX_start(ctx);
  ret = BN_CTX_get(ctx);
  n = BN_CTX_get(ctx);
  iVar2 = BN_num_bits(*(BIGNUM **)(param_4 + 0x10));
  iVar7 = iVar2 + 0xe;
  if (-1 < iVar2 + 7) {
    iVar7 = iVar2 + 7;
  }
  len = iVar7 >> 3;
  to = (uchar *)CRYPTO_malloc(len,"rsa_eay.c",0x171);
  if ((ret == (BIGNUM *)0x0 || n == (BIGNUM *)0x0) || (to == (uchar *)0x0)) {
    sVar6 = 0xffffffff;
    ERR_put_error(4,0x66,0x41,"rsa_eay.c",0x174);
    BN_CTX_end(ctx);
    BN_CTX_free(ctx);
    if (to == (uchar *)0x0) goto LAB_08109e68;
  }
  else {
    if (param_5 != 3) {
      if (param_5 == 5) {
        iVar7 = RSA_padding_add_X931(to,len,param_2,(int)param_1);
        goto LAB_08109e9d;
      }
      if (param_5 == 1) {
        iVar7 = RSA_padding_add_PKCS1_type_1(to,len,param_2,(int)param_1);
        goto LAB_08109e9d;
      }
      iVar2 = 0x185;
      iVar7 = 0x76;
      goto LAB_08109e2a;
    }
    iVar7 = RSA_padding_add_none(to,len,param_2,(int)param_1);
LAB_08109e9d:
    if ((iVar7 < 1) ||
       (ctx_00 = (char *)ret, pBVar3 = BN_bin2bn(to,len,ret), pBVar3 == (BIGNUM *)0x0))
    goto LAB_08109e36;
    iVar7 = BN_ucmp(ret,*(BIGNUM **)(param_4 + 0x10));
    if (iVar7 < 0) {
      uVar4 = *(uint *)(param_4 + 0x3c);
      local_4c = (BN_BLINDING *)0x0;
      local_48 = (BIGNUM *)0x0;
      if ((uVar4 & 0x80) == 0) {
        local_4c = (BN_BLINDING *)rsa_get_blinding();
        if (local_4c == (BN_BLINDING *)0x0) {
          iVar2 = 0x198;
          iVar7 = 0x44;
        }
        else {
          local_48 = BN_CTX_get(ctx);
          if (local_48 != (BIGNUM *)0x0) {
            CRYPTO_lock((int)local_48,9,(char *)0x19,(int)"rsa_eay.c");
            iVar7 = BN_BLINDING_convert_ex(ret,local_48,local_4c,ctx);
            param_1 = (BN_CTX *)0x14b;
            ctx_00 = "rsa_eay.c";
            CRYPTO_lock(iVar7,10,(char *)0x19,(int)"rsa_eay.c");
            if (iVar7 == 0) goto LAB_08109e36;
            uVar4 = *(uint *)(param_4 + 0x3c);
            goto LAB_08109ee8;
          }
          iVar2 = 0x1a1;
          iVar7 = 0x41;
        }
        goto LAB_08109e2a;
      }
LAB_08109ee8:
      if (((uVar4 & 0x20) == 0) &&
         ((((*(int *)(param_4 + 0x1c) == 0 || (*(int *)(param_4 + 0x20) == 0)) ||
           (*(int *)(param_4 + 0x24) == 0)) ||
          ((*(int *)(param_4 + 0x28) == 0 || (*(int *)(param_4 + 0x2c) == 0)))))) {
        if ((uVar4 & 0x100) == 0) {
          pBVar3 = &local_34;
          BN_init(pBVar3);
          ppuVar1 = *(ulong ***)(param_4 + 0x18);
          local_34.d = *ppuVar1;
          local_34.top = (int)ppuVar1[1];
          local_34.dmax = (int)ppuVar1[2];
          local_34.neg = (int)ppuVar1[3];
          local_34.flags = (uint)ppuVar1[4] & 0xfffffffe | local_34.flags & 1U | 6;
          uVar4 = *(uint *)(param_4 + 0x3c);
        }
        else {
          pBVar3 = *(BIGNUM **)(param_4 + 0x18);
        }
        if ((uVar4 & 2) != 0) {
          ctx_00 = *(char **)(param_4 + 0x10);
          param_1 = ctx;
          pBVar5 = BN_MONT_CTX_set_locked
                             ((BN_MONT_CTX **)(param_4 + 0x40),(int)(BN_MONT_CTX **)(param_4 + 0x40)
                              ,(BIGNUM *)0x9,(BN_CTX *)ctx_00);
          if (pBVar5 == (BN_MONT_CTX *)0x0) goto LAB_08109e36;
        }
        iVar7 = (**(code **)(*(int *)(param_4 + 8) + 0x18))
                          (n,ret,pBVar3,*(undefined4 *)(param_4 + 0x10),ctx,
                           *(undefined4 *)(param_4 + 0x40),ctx_00,param_1);
        if (iVar7 != 0) goto LAB_08109f3b;
        goto LAB_08109e36;
      }
      iVar7 = (**(code **)(*(int *)(param_4 + 8) + 0x14))(n,ret,param_4,ctx);
      if (iVar7 == 0) goto LAB_08109e36;
LAB_08109f3b:
      if ((local_4c != (BN_BLINDING *)0x0) &&
         (iVar7 = BN_BLINDING_invert_ex(n,local_48,local_4c,ctx), iVar7 == 0)) goto LAB_08109e36;
      local_50 = n;
      if (param_5 == 5) {
        BN_sub(ret,*(BIGNUM **)(param_4 + 0x10),n);
        iVar7 = BN_cmp(n,ret);
        local_50 = ret;
        if (iVar7 == 0) {
          local_50 = n;
        }
      }
      iVar2 = BN_num_bits(local_50);
      iVar7 = iVar2 + 7;
      if (iVar2 + 7 < 0) {
        iVar7 = iVar2 + 0xe;
      }
      iVar7 = BN_bn2bin(local_50,(uchar *)((len - (iVar7 >> 3)) + (int)param_3));
      sVar6 = len;
      if (0 < (int)(len - iVar7)) {
        memset(param_3,0,len - iVar7);
      }
    }
    else {
      iVar2 = 399;
      iVar7 = 0x84;
LAB_08109e2a:
      ERR_put_error(4,0x66,iVar7,"rsa_eay.c",iVar2);
LAB_08109e36:
      sVar6 = 0xffffffff;
    }
    BN_CTX_end(ctx);
    BN_CTX_free(ctx);
  }
  OPENSSL_cleanse(to,len);
  CRYPTO_free(to);
LAB_08109e68:
  if (local_20 == *(int *)(in_GS_OFFSET + 0x14)) {
    return sVar6;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}

