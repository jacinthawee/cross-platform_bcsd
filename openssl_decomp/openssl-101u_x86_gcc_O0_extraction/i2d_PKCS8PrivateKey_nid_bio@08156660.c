...
/* WARNING: Removing unreachable block (ram,0x08156410) */
/* WARNING: Removing unreachable block (ram,0x08156440) */
...

{
  ...
  if (a == (PKCS8_PRIV_KEY_INFO *)0x0) {
    ...
    if (kstr == (char *)0x0) {
      ...
      p8 = PKCS8_encrypt(nid,(EVP_CIPHER *)0x0,acStack_420,len,(uchar *)0x0,0,0,a);
      OPENSSL_cleanse(acStack_420,len);
    }
    else {
      p8 = PKCS8_encrypt(nid,(EVP_CIPHER *)0x0,kstr,klen,(uchar *)0x0,0,0,a);
    }
    PKCS8_PRIV_KEY_INFO_free(a);
    if (p8 == (X509_SIG *)0x0) {
      iVar1 = 0;
